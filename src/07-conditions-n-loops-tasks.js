...
function getSumBetweenNumbers(n1, n2) {
  function getInterval(start, end) {
    ...
    return new Array(len).fill(start).map((elem, i) => elem + i);
  }
  return getInterval(n1, n2).reduce((a, b) => a + b);
}
/**
...
function isTriangle(a, b, c) {
  ...
  const cond2 = sort[2] === sort[0] && sort[2] === sort[1];
  const cond1 = sort[2] ** 2 === sort[0] ** 2 + sort[1] ** 2;
...
function doRectanglesOverlap(rect1, rect2) {
  return !(
    ...
    || rect2.height + rect1.top < rect1.top
...
function findFirstSingleChar(str) {
  let i = 0;
  while (i < str.length) {
    const sub1 = str.slice(i + 1);
    const sub2 = str.slice(0, i);
    if (!(sub2 + sub1).includes(str[i])) {
      return str[i];
    }
    i += 1;
  }
